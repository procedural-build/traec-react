...
export default class CompanyAuthGroupForm extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            actions: initActionList.reduce((obj, i) => {obj[i]=true;return obj}, {})
        }
    ...
    render_actions_table_rows() {
        ...
        for (let [objKey, name] of Object.entries(this.viewObjectTypes)) {
            let colCells = this.operations.map((operation, i) => {
                ...
                return (
                    <td key={counter++} className="text-center" 
                        style={{backgroundColor: isChecked ? "#99EB99" : "#EB9999"}}
                    >
            ...
            // Add the row header
            colCells.unshift(<th key={counter++} scope="row">{name}</th>)
    ...
    render_actions_table() {
        if (this.state.is_admin) { return null; }
        let colHeadings = this.operations.map((o, i) => (<th key={i} scope="col" width="15%" className="text-center">{o}</th>))
        colHeadings.unshift(<th key={colHeadings.length} scope="col"></th>)
        return(
            <div className="form-group row mb-0">
            <div className="col-sm-12">
            <table className="table table-sm">
                <thead><tr>{colHeadings}</tr></thead>
                <tbody>
                    {this.render_actions_table_rows()}
                </tbody>
            </table>
            </div>
            </div>
            ...
            <form>
                <div className="form-group row mb-0">
                    <div className="col-sm-11">
                        <input 
                            type="text" name="name" className="form-control-plaintext" 
                    ...
                    <div className="form-check col-sm-1">
                        <input className="form-check-input" type="checkbox" id="adminCheck" 
                ...
                <button className="btn btn-sm btn-default"
                    onClick={e => this.props.showFormHandler(e, false)}
                >
                    Close
                </button>
                <button 
