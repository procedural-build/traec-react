...
export default function(state = initialState, action) {
    ...
    switch (action.type){
        case types.LOGIN_SUCCESS:
            ...
            return state.merge(Im.fromJS(action.payload), Im.fromJS({isAuthenticated: true, status: 'confirmed', decoded_token}))
        case types.LOGIN_STATUS:
            return state.merge(Im.fromJS(action.payload))
        case types.LOGIN_FAILURE:
            //console.log("LOGIN FAILURE", action.payload)
            localStorage.removeItem('token')
            return state.merge(Im.fromJS(action.payload), Im.fromJS({
                isAuthenticated: false, 
                token: null,
        ...
        case types.REGISTER_SUCCESS:
            return state.mergeIn(['registration', ], Im.fromJS({
        ...
        case types.ACTIVATE_SUCCESS:
            return state.mergeIn(['registration', 'activate'], Im.fromJS({
                status: 'confirmed'
            }))
        ...
        case types.RESET_SUCCESS:
            return state.mergeIn(['registration', 'password_reset'], Im.fromJS({
                status: 'confirmed'
            }))
