...
export class BSForm extends React.Component {
    ...
    renderTextInput (item, keyIndex) {
        ...
        return (
            ...
            <div className={`form-group ${extraClass}`}>
                {labelBlock}
                <input type={inputType} className={`form-control ${validClass}`} 
                    placeholder={details.placeholder || ''}
                    name={item.name} onChange={this.handleChange} value={value} formNoValidate={true}/> 
                {error}
            </div>
            {rowBreakDiv}
            </React.Fragment>
    ...
    renderTextAreaInput (item, keyIndex) {
        ...
        return (
            ...
            <div className={`form-group ${extraClass}`}>
                {labelBlock}
                <textarea className={`form-control ${validClass}`} 
                    placeholder={details.placeholder || ''} rows={rows}
                    name={item.name} onChange={this.handleChange} value={value} /> 
                {error}
            </div>
            {rowBreakDiv}
            </React.Fragment>
    ...
    renderSelectInput (item, keyIndex) {
        ...
        return (
            ...
            <div className={`form-group ${extraClass}`}>
                {labelBlock}
                <select className={`custom-select ${validClass}`} 
                    onChange={this.handleChange} name={item.name}
                    value={value}>
                    {details.options}
                </select> 
                {error}
    ...
    renderDateInput (item, keyIndex) {
        ...
        const rowBreakDiv = details.endRow ? <div className="w-100" /> : ""
        const labelBlock = details.label === '' ? '' : <label>{label}</label>
        const helpTextBlock = details.helpText ? (<small id={`${label}_help`} className="form-text text-muted">{details.helpText}</small>) : null
        return (
            <React.Fragment key={keyIndex}>
                <div className={`form-group ${extraClass}`}>
                    {labelBlock}
                    ...
                    <DatePicker 
                        ...
                        onChange={value => this.handleChange({target: {
                            value: value, 
                            name: item.name
                        }})}
    ...
    render_non_field_errors() {
        ...
        if (errors && errors.hasOwnProperty("non_field_errors") && errors.non_field_errors.length) {
            let error_list = errors.non_field_errors.map( (error, i) => 
                <p key={i} className="help-block">{error}</p>
            )
            non_field_errors = 
