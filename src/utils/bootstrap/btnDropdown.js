...
export class BSBtnDropdown extends React.Component {
    ...
    renderItem(item, i) {
        if (item.name != undefined) {
            if ('onClick' in item) {
                return (<a key={i} className="dropdown-item" onClick={(e) => item.onClick(e, item.id)}>{item.name}</a>);
            } else if ('linkTo' in item) {
                return (<Link key={i} to={item.linkTo} className="dropdown-item">{item.name}</Link>);
            } else if ('download' in item) {
                return (<a key={i} className="dropdown-item" href={item.href} download={item.download}>{item.name}</a>);
            } else if ('href' in item) {
                return (<a key={i} className="dropdown-item" href={item.href}>{item.name}</a>);
            }
        } else {
    ...
    floatStyle() {
        if (this.props.floatStyle == null) {
            return "float-right";
        } else { return this.props.floatStyle}
    }

    getClass(links) {
        if (!this.hasItems(links)) { return ""; }
    ...
    render() {
        ...
        return (
            <div className={`btn-group ${this.floatStyle()}`} role="group" aria-label="proj-admin-menu">    
                <a className={this.getClass(links)} id="btnDropdown" role="button" data-toggle={this.data_toggle(links)} aria-haspopup="true" aria-expanded="false">
                    {this.menuHeader()}
                </a>
