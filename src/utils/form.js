...
class BaseForm extends React.Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (newItem && newItem.get('errors')) {
            ...
            fields = Im.List(fields).reduce( (a, k) => { a[k] = 1; return a;}, {})
    ...
    render_close() {
        if (this.props.forceShowForm) { return null }
        return (<button type="button" onClick={this.props.toggleForm} className="btn btn-sm btn-default float-left">Close</button>)
    }

    render(){
        let {showForm, forceShowForm} = this.props
        if (!showForm && !forceShowForm) { return "" }
        ...
        <div className="col-sm-12 ">
            <form onSubmit={this.onSubmit}>
                ...
                {this.render_close()}
                <button type="submit" className="btn btn-sm btn-primary float-right">Submit</button>
...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    return {
        ...
        toggleForm: () => { dispatch(toggleForm(s)) },
        dispatch: dispatch
    }
}
